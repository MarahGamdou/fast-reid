...
class Market1501Attr(Dataset):
    ...
    def __init__(self, root="datasets", market1501_500k=False, **kwargs):
        ...
        market_attr = mat4py.loadmat(osp.join(self.data_dir, "market_attribute.mat"))[
            "market_attribute"
        ]
        ...
        super(Market1501Attr, self).__init__(
            train, val, test, attr_dict=attr_dict, **kwargs
        )
    ...
    def process_dir(self, dir_path, annotation, sorted_attrs):
        ...
        for img_path in img_paths:
            ...
            attrs = np.array(
                [int(annotation[i][img_index]) - 1 for i in sorted_attrs],
                dtype=np.float32,
            )
