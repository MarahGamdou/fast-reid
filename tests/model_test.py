...
class MyTestCase(unittest.TestCase):
    def test_se_resnet101(self):
        ...
        cfg.MODEL.BACKBONE.PRETRAIN_PATH = (
            "/export/home/lxy/.cache/torch/checkpoints/se_resnet101_ibn_a.pth.tar"
        )

        net1 = build_resnet_backbone(cfg)
        net1.cuda()
        net2 = nn.DataParallel(se_resnet101_ibn_a())
        res = net2.load_state_dict(
            torch.load(cfg.MODEL.BACKBONE.PRETRAIN_PATH)["state_dict"], strict=False
        )
        net2.cuda()
